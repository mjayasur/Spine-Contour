...
Brainstorming and planning are **complete and approved**. No implementation code has
been written yet. The last three commits are documentation only:

```
acc91df  make CSV column mapping user-editable
715aa88  add implementation plans
304b1e1  add UI redesign spec
7aa1a86  use workflow release publishing   <- last pre-redesign commit
```

The working tree is otherwise identical to the production app at `7aa1a86`.
...
|---|---|---|---|
| 01 | `2026-08-31-01-preview-build.md` | 4 | Preview installs beside the real app |
...
- **Plan 01, Task 4** — enabling GitHub Actions on the fork (a browser click), then
  installing both apps and confirming Windows shows two separate uninstall entries.
  This is the task that validates the project's central risk. Do not mark it complete
  on a partial check.
